...
const AccumalatedGoldSummary = () => {
  const [categories, setAccumalatedGoldSummary] = useState([]);
  useEffect(() => {
    const fetchcategories = async () => {
      const { data } = await axios.get(`${BASE_URL}/api/Category`);

      setAccumalatedGoldSummary(data);
    };
    fetchcategories();
  }, []);
  return (
...
export default AccumalatedGoldSummary;
