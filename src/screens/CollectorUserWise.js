...
const CollectorUserWise = () => {
  const [categories, setCollectorUserWise] = useState([]);
  useEffect(() => {
    const fetchcategories = async () => {
      const { data } = await axios.get(`${BASE_URL}/api/Category`);

      setCollectorUserWise(data);
    };
    fetchcategories();
  }, []);
  return (
...
export default CollectorUserWise;
