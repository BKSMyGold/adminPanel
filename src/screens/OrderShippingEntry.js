...
const OrderShippingEntry = () => {
  const [transactions, setTransactions] = useState([])
  const [deliveryCharges, setDeliveryCharges] = useState(null)
  const [totalAmounts, setTotalAmounts] = useState(null)
  useEffect(() => {
    const fetchcategories = async () => {
      const { data } = await axios.get('http://13.59.57.74:5000/api/order/transactions')
      data = data ? data.filter(transaction => transaction.transactions.status === "Order Completed" && transaction.consignment) : []
...
export default OrderShippingEntry
