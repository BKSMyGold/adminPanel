...
const SellInStoreDue = () => {
  const [sellInStoreDue, setSellInStoreDue] = useState([])
  useEffect(() => {
    const fetchcategories = async () => {
      const { data:response } = await axios.get('http://13.59.57.74:5000/api/appointment')
      setSellInStoreDue(response.data.filter(appointment => appointment.status === 'Offer Accepted'))
      // setSellInStoreDue(response.data) // Uncomment this to get all the data
    }
    fetchcategories()
  }, [])
  return (
                    ...
                    <table class='table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3'>
                      ...
                      <thead>
                        <tr class='fw-bolder text-muted'>
                          ...
                          <th class='min-w-120px'>Wieght</th>
                      ...
                      {/*begin::Table body*/}
                      <tbody>
                        {sellInStoreDue.map((appointment) => (
                          <tr>
                            <td>
                              {appointment.id}
                            </td>
